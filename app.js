...
Bot.prototype.getMove = function(input) {
  for(var i=0; i < numPre; i++){
      ...
      var bpp = beat[pp]
      var bbpp = beat[bpp]
      this.pScore[0][i]=0.9*pScore[0][i]+((input==pp)-(input==bbpp))*3
      this.pScore[1][i]=0.9*pScore[1][i]+((output==pp)-(output==bbpp))*3
      this.pScore[2][i]=0.87*pScore[2][i]+(input==pp)*3.3-(input==bpp)*1.2-(input==bbpp)*2.3
      this.pScore[3][i]=0.87*pScore[3][i]+(output==pp)*3.3-(output==bpp)*1.2-(output==bbpp)*2.3
      this.pScore[4][i]=(pScore[4][i]+(input==pp)*3)*(1-(input==bbpp))
      this.pScore[5][i]=(pScore[5][i]+(output==pp)*3)*(1-(output==bbpp))
    }
  for(var i=0; i<numMeta; i++){
      this.mScore[i]=0.96*(mScore[i]+(input==m[i])-(input==beat[beat[m[i]]])) + (Math.random()-0.5)*dithering
    }
  this.soma[centrifuge[input+output]] +=1;
  this.rps[centripete[input]] +=1;
  this.moves[0]+=(centrifuge[input+output]).toString()
  this.moves[1]+=input
  this.moves[2]+=output
  this.length+=1
  for(var y = 0; y<3; y++){
      ...
      i = moves[y].lastIndexOf(moves[y].slice(length-j,length),0,length-1)
      this.p[0+2*y] = moves[1][j+i]
      this.p[1+2*y] = beat[moves[2][j+i]]
    }
  j=Math.min([length,limit])
  while(j>=2 && !(moves[0].slice(0,length-2).contains(moves[0].slice(length-j,length-1)))){
      j-=1
  ...
  if (j+i>=length){
    ...
    this.p[6] = moves[1][j+i]
    this.p[7] = beat[moves[2][j+i]]
    }

  this.best[0] = soma[centrifuge[output+'R']]*rps[0]/rps[centripete[output]]
  this.best[1] = soma[centrifuge[output+'P']]*rps[1]/rps[centripete[output]]
  this.best[2] = soma[centrifuge[output+'S']]*rps[2]/rps[centripete[output]]
  this.p[8] = this.p[9] = a[best.indexOf(Math.max(best))]

  for(var i = 10; i<numPre; i++){
    this.p[i]=beat[beat[p[i-10]]]
    }

  for(var i = 0; i<numMeta; i+=2){
    this.m[i]=       p[pScore[i  ].indexOf(Math.max(pScore[i  ]))]
    this. m[i+1]=beat[p[pScore[i+1].indexOf(Math.max(pScore[i+1]))]]
    }
  }
  output = beat[m[mScore.indexOf(Math.max(mScore))]]
  if (Math.max(mScore)<3+Math.random() || getRandomInt(3,40)>length || Math.random() < 0.5) {
